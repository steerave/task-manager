...
Each task in the note is a clickable wiki-link to its source file. Checking off a task checkbox in Obsidian and running `task today` again will automatically mark that task as done. The daily note also auto-regenerates after every task mutation (`add`, `done`, `update`, `delete`, `waiting`, `priority`) so it stays current without needing to rerun `task today`.
